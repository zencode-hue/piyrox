 ...
 * - Deletes fake users (emails matching customer_*@piyrox.sbs and reviews@piyrox.sbs)
 */
export async function GET() {
  ...
  const seedUsers = await db.user.findMany({
    where: {
      OR: [
        { email: { startsWith: "customer_", endsWith: "@piyrox.sbs" } },
        { email: "reviews@piyrox.sbs" },
      ],
    },
  ...
  let ghost = await db.user.findFirst({ where: { email: "ghost@piyrox.sbs" } });
  if (!ghost) {
    ghost = await db.user.create({
      data: {
        email: "ghost@piyrox.sbs",
