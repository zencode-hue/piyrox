...
export default function ProductActions({
  ...
  const [wishlisted, setWishlisted] = useState(() => {
    ...
    return (JSON.parse(localStorage.getItem("piyrox_wishlist") ?? "[]") as string[]).includes(productId);
  });
  ...
  function toggleWishlist() {
    const list = JSON.parse(localStorage.getItem("piyrox_wishlist") ?? "[]") as string[];
    const next = wishlisted ? list.filter((id) => id !== productId) : [...list, productId];
    localStorage.setItem("piyrox_wishlist", JSON.stringify(next));
