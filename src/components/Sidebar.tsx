  ...
  const renderGroup = (label: string, items: Chat[]) => {
    ...
    return (
      <div key={label} className="mb-4">
        <div className="text-xs font-semibold text-gray-500 px-3 mb-1">{label}</div>
        {items.map((chat) => (
          <div
            key={chat.id}
          ...
          >
            ...
            {(hoveredId === chat.id || chat.id === activeChatId) && (
              <button
                ...
                className="ml-2 p-1 rounded hover:bg-white/10 text-gray-500 hover:text-gray-300 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity"
  ...
  return (
    <aside
      className={`
        ...
        flex flex-col h-full w-[260px] bg-[#171717] border-r border-white/[0.06]
    ...
    >
      ...
      <div className="flex items-center justify-between p-3 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-white">PiyRox Chat</span>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="px-3 mb-3 flex-shrink-0">
        <button
          onClick={onNewChat}
          className="flex items-center gap-2 w-full px-3 py-2.5 rounded-lg border border-white/[0.08] hover:bg-white/[0.05] transition-colors text-sm text-gray-300 hover:text-white"
        >
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="p-3 border-t border-white/[0.06] flex-shrink-0">
        <a
          ...
          className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white/[0.05] transition-colors text-sm text-gray-400 hover:text-gray-200"
        >
